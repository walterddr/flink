...
import org.apache.flink.streaming.api.datastream.{OverStream => JavaOStream}
...
/**
 * A [[OverStream]] represents a data stream where elements are grouped by
 * key, and for each key, the stream of elements is assigned to a rolling
 * same-window
 */
@Public
class OverStream[T, K](javaStream: JavaOStream[T, K]) {
  ...
  /**
    ...
    */
  @PublicEvolving
  def allowedLateness(lateness: Time): OverStream[T, K] = {
...
//   *
//   * You can get the stream of late data using [[DataStream.getSideOutput()]]
//   * on the [[DataStream]] resulting from the windowed operation with the same [[OutputTag]].
//   */
//  @PublicEvolving
...
//   * For example, tumbling time windows can perfectly pre-aggregate the data, meaning that only
//   * one element per key is stored. Sliding time windows will pre-aggregate on the granularity
//   * of the slide interval, so a few elements are stored per key (one per slide interval).
...
//  }
  ...
  def process[R: TypeInformation](
      function: ProcessWindowFunction[T, R, K, VoidWindow]): DataStream[R] = {

    val cleanFunction = clean(function)
    val applyFunction = new ScalaProcessWindowFunctionWrapper[T, R, K, VoidWindow](cleanFunction)
  ...
  def apply[R: TypeInformation](
      function: WindowFunction[T, R, K, VoidWindow]): DataStream[R] = {
    
    val cleanFunction = clean(function)
    val applyFunction = new ScalaWindowFunctionWrapper[T, R, K, VoidWindow](cleanFunction)
   ...
   */
  def apply[R: TypeInformation](
      function: (K, VoidWindow, Iterable[T], Collector[R]) => Unit): DataStream[R] = {
    if (function == null) {
      throw new NullPointerException("WindowApply function must not be null.")
    }

    val cleanedFunction = clean(function)
    val applyFunction = new ScalaWindowFunction[T, R, K, VoidWindow](cleanedFunction)
...
//        new SumAggregator(
//            position, jStream.getInputType, jStream.getExecutionEnvironment.getConfig)
//
//      case _ =>
