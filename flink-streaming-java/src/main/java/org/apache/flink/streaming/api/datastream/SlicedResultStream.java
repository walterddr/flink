 ...
 *
 * <p>Noted that {@code SlicedResultStream} is  product of a {@link WindowOperator} with a
 ...
 */
@Public
public class SlicedResultStream<T, K, W extends Window> {
	...
	@PublicEvolving
	public SlicedResultStream(KeyedStream<Slice<T, K, W>, K> input) {
		this.input = input;
	 ...
	 */
	@PublicEvolving
	public SlicedResultStream<T, K, W> trigger(Trigger<? super Slice<T, K, W>, ? super W> trigger) {
...
//	}
	...
	private <ACC, R> AggregateFunction<Slice<T, K, W>, ACC, R> wrapSliceFunction(AggregateFunction<T, ACC, R> aggregateFunction) {
		return new AggregateFunction<Slice<T, K, W>, ACC, R>() {
			@Override
			...
			@Override
			public ACC add(Slice<T, K, W> value, ACC accumulator) {
				return aggregateFunction.add(value.getContent(), accumulator);
			}

			@Override
			public R getResult(ACC accumulator) {
	...
	private ReduceFunction<Slice<T, K, W>> wrapSliceFunction(ReduceFunction<T> reduceFunction) {
		return (ReduceFunction<Slice<T, K, W>>)
			(a, b) -> new Slice<>(
				reduceFunction.reduce(a.getContent(), b.getContent()),
				a.getKey(),
				a.getWindow()
			);
	}
