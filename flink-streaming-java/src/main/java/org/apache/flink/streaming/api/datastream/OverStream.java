 ...
 */
@Public
public class OverStream<T, K> {

	/** The keyed data stream that is windowed by this stream. */
	private final KeyedStream<T, K> input;

	/** The window assigner. */
	private final SameWindowAssigner<? super T> windowAssigner;

	/** The trigger that is used for window evaluation/emission. */
	private Trigger<? super T, ? super VoidWindow> trigger;

	/** The evictor that is used for evicting elements before window evaluation. */
	private final Evictor<? super T, ? super VoidWindow> evictor;
	...
	@PublicEvolving
	public OverStream(KeyedStream<T, K> input,
					  SameWindowAssigner<? super T> sameWindowAssigner,
					  Evictor<? super T, ? super VoidWindow> evictor) {
		Preconditions.checkNotNull(evictor);
		this.input = input;
		this.windowAssigner = sameWindowAssigner;
	 ...
	 */
	@PublicEvolving
	public OverStream<T, K> allowedLateness(Time lateness) {
...
//	 */
//	@PublicEvolving
//	public OverStream<T, K, W> sideOutputLateData(OutputTag<T> outputTag) {
...
//		return reduce(function, new PassThroughWindowFunction<K, VoidWindow, T>());
//	}
//
//	/**
...
//			WindowFunction<T, R, K, ? super VoidWindow> function) {
//
...
//
//		final String opName = generateOperatorName(evictor, reduceFunction, function);
...
//
//		// TODO: FIX ME
//		KeyedProcessFunction<K, T, R> overProcessFunction = new KeyedProcessFunction<K, T, R>() {
//
//			@Override
...
//		};
//
//		return input.process(overProcessFunction, resultType);
//	}
//
//
//	/**
...
//		checkNotNull(function, "aggregate function cannot be null");
//
...
//
//		return aggregate(function, new PassThroughWindowFunction<K, VoidWindow, R>(),
...
//			WindowFunction<V, R, K, ? super VoidWindow> windowFunction) {
//
...
//
//		final String opName = generateOperatorName(evictor, aggregateFunction, windowFunction);
//		KeySelector<T, K> keySel = input.getKeySelector();
//
//		// TODO: FIX ME
//		KeyedProcessFunction<K, T, R> overProcessFunction = new KeyedProcessFunction<K, T, R>() {
...
//	}
	 ...
	 * @return The data stream that is the result of applying the window function to the window.
	 */
	public <R> SingleOutputStreamOperator<R> apply(WindowFunction<T, R, K, ? super VoidWindow> function) {
	 ...
	 * @return The data stream that is the result of applying the window function to the window.
	 */
	public <R> SingleOutputStreamOperator<R> apply(WindowFunction<T, R, K, ? super VoidWindow> function, TypeInformation<R> resultType) {
		function = input.getExecutionEnvironment().clean(function);
	 ...
	 */
	@PublicEvolving
	public <R> SingleOutputStreamOperator<R> process(ProcessWindowFunction<T, R, K, ? super VoidWindow> function) {
	 ...
	 */
	@Internal
	public <R> SingleOutputStreamOperator<R> process(ProcessWindowFunction<T, R, K, ? super VoidWindow> function, TypeInformation<R> resultType) {
		function = input.getExecutionEnvironment().clean(function);
		return apply(new InternalIterableProcessWindowFunction<>(function), resultType, function);
	}

	@SuppressWarnings("unchecked")
	private <R> SingleOutputStreamOperator<R> apply(InternalWindowFunction<Iterable<T>, R, K, ? super VoidWindow> function, TypeInformation<R> resultType, Function originalFunction) {
		final String opName = generateOperatorName(windowAssigner, trigger, evictor, originalFunction, null);
		KeySelector<T, K> keySel = input.getKeySelector();

		WindowOperator<K, T, Iterable<T>, R, ? super VoidWindow> operator;
		...
		operator =
			new EvictingWindowOperator<>(windowAssigner,
				...
				input.getKeyType().createSerializer(getExecutionEnvironment().getConfig()),
				stateDesc,
				(InternalWindowFunction<Iterable<T>, R, K, VoidWindow>) function,
				trigger,
				evictor,
				allowedLateness,
				lateDataOutputTag);
