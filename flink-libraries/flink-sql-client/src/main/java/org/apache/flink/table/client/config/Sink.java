...
/**
 * Configuration of a table sink. Parses an entry in the `sink` list of an environment
...
public class Sink extends TableSinkDescriptor {
	...
	private Sink(String name, Map<String, String> properties) {
		this.name = name;
	...
	public static Sink create(Map<String, Object> config) {
		if (!config.containsKey(NAME)) {
			throw new SqlClientException("The 'name' attribute of a table sink is missing.");
		}
		final Object name = config.get(NAME);
		if (name == null || !(name instanceof String) || ((String) name).length() <= 0) {
			throw new SqlClientException("Invalid table sink name '" + name + "'.");
		}
		final Map<String, Object> properties = new HashMap<>(config);
		properties.remove(NAME);
		return new Sink((String) name, ConfigUtil.normalizeYaml(properties));
	}
