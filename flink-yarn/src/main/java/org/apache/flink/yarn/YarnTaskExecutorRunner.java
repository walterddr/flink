...
public class YarnTaskExecutorRunner {
	...
	private static void setupConfigurationFromVariables(Configuration configuration, String currDir, Map<String, String> variables) throws IOException {
		...
		final String keytabPathObtained = variables.get(YarnConfigKeys.KEYTAB_PATH);
		LOG.info("TM: keytab path obtained {}", keytabPathObtained);
		...
		if (keytabPathObtained != null) {
			// Resolving keytab path relative to working directory
			File f = new File(currDir, keytabPathObtained);
			if (!f.exists()) {
				// try absolute path resolution
				f = new File(keytabPathObtained);
			}
