...
public class RestApiTest {
	private static final Duration TEST_TIMEOUT = Duration.of(10L, ChronoUnit.SECONDS);
	private static final ObjectMapper MAPPER = new ObjectMapper();
	private static final ObjectMapper YAML_MAPPER = new ObjectMapper(new YAMLFactory());
	private static final Joiner JOINER = Joiner.on("\n");

	private static int testSuccessCount = 0;
	private static List<String> configuredSkipList = new ArrayList<>();
	...
	public static void main(String[] args) throws Exception {
		...
		checkpointPath = params.getRequired("checkpointPath");
		...
		// Wait for all the content of the exception to sink in.
		Thread.sleep(1000L);
	...
	private static void validateRunningFlinkJob(HttpTestClient httpClient) {
		...
		try {
			...
			requestBody = new SavepointDisposalRequest(checkpointPath);
			...
			requestBody = new SavepointTriggerRequestBody(checkpointPath, false);
...
//					"rescaling/" + rescalingResponse.getTriggerId().toString()));
	...
	private static void loadResource() {
		...
		try (InputStream in = RestApiTest.class.getClassLoader().getResourceAsStream("testResource.yaml")) {
			JsonNode jsonNode = YAML_MAPPER.readTree(in);
			for (final JsonNode requestBodyNode : jsonNode.get("requestBodies")) {
				requestBodyMap.put(requestBodyNode.get("url").asText(),
					resolvePayloadParameter(requestBodyNode.get("payload").asText()));
			}
			for (final JsonNode skipItem : jsonNode.get("skipList")) {
				configuredSkipList.add(skipItem.asText());
			}
		} catch (Exception e) {
	...
	private static String resolvePayloadParameter(String payload) {
		return payload
			.replace("${savepointPath}", checkpointPath);
	}
	...
	private static <R extends RequestBody, P extends ResponseBody, M extends MessageParameters>
		...
		try {
			...
			if (configuredSkipList.contains(spec.getTargetRestEndpointURL())) {
				testSkipList.add(
					"URL: " + spec.getTargetRestEndpointURL() + "\n"
						+ "METHOD: " + spec.getHttpMethod() + "\n"
						+ "EXCEPTION: " + e + "\n"
